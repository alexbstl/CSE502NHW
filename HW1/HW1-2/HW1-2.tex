...
\While{$ j < n $} \Comment{n times}
    \State $S_1$  \Comment{n-1 times}
	\State $k \gets n$ \Comment{n-1 times}
	\While{$k > j$} \Comment{n-j+1 times}
		\State $S_2$ \Comment{n-j times}
		\State $k--$ \Comment{n-j times}
	\EndWhile
...
\While{$j \leq n$} \Comment{n+2 times}
	\State $k \gets 0$ \Comment{n+1 times}
...
\While{$j \leq n$} \Comment{n+1 times}
	\State $k \gets 1$ \Comment{n times}
